...
export const employeeSchema = z.object({
    ...
    type: z.enum(["full-time", "part-time", "casual", "contractor"]),
    baseHourlyRate: z.number().min(0, "Rate must be positive"),
    superRate: z.number().min(0).max(1, "Super rate must be between 0 and 1"),
    bankBsb: z.string().regex(/^\d{6}$/, "BSB must be 6 digits").nullable().or(z.literal("")),
    bankAccount: z.string().min(6).max(20).nullable().or(z.literal("")),
});
